...
export function ServicesList({ search }: ServicesListProps) {
  ...
  const filteredServices = servicesArray.filter((service, index, self) => {
    // Remove duplicates by checking if this is the first occurrence of the service
    const isFirstOccurrence = self.findIndex(s => s.id === service.id) === index;
