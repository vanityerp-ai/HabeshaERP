...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      // Get location name and staff name for reference
      ...
      // Get staff name for transaction metadata
      ...
      // NEW: Calculate total cost and profit for products (matching Online Store parity)
      const productItems = data.items.filter((item: any) => item.type === "product");
      const totalProductCost = productItems.reduce((sum: number, item: any) => {
        const itemCost = item.cost || 0;
        return sum + (itemCost * item.quantity);
      }, 0);
      const productAmount = hasProducts ? data.items
        .filter((item: any) => item.type === "product")
        .reduce((sum: number, item: any) => sum + (item.price * item.quantity), 0) : 0;
      const productProfit = productAmount - totalProductCost;
      ...
      await prisma.transaction.create({
        data: {
          ...
          description: `POS Sale - ${data.items.length} item(s) at ${locationName}`, // UPDATED: Include location
          ...
          // NEW: Add metadata for cost tracking and profit calculation (matching Online Store)
          metadata: JSON.stringify({
            totalProductCost: hasProducts ? totalProductCost : null,
            productProfit: hasProducts ? productProfit : null,
            source: "pos",
