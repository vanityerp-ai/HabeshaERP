...
export function TransactionProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (isInitialized && transactions.length > 0) {
      console.log('🔄 TRANSACTION PROVIDER: Syncing transactions to database');

      // Sync transactions to database asynchronously
        ...
        try {
          for (const transaction of transactions) {
            await fetch('/api/transactions', {
              ...
              console.warn(`Failed to sync transaction ${transaction.id} to database:`, error);
            });
          }
          console.log(`✅ TRANSACTION PROVIDER: Synced ${transactions.length} transactions to database`);
        } catch (error) {
          console.error('❌ TRANSACTION PROVIDER: Error syncing to database:', error);
        }
      };
          ...
          if (response.ok) {
            ...
            if (posSalesToSync.length > 0) {
              ...
              for (const prismaTx of posSalesToSync) {
                try {
                  // Parse metadata to get staff name and other details
                  let parsedMetadata = {};
                  if (prismaTx.metadata) {
                    try {
                      parsedMetadata = typeof prismaTx.metadata === 'string'
                        ? JSON.parse(prismaTx.metadata)
                        : prismaTx.metadata;
                    } catch (e) {
                      console.warn(`Failed to parse metadata for transaction ${prismaTx.id}`);
                    }
                  }
                  ...
                  const convertedTx = {
                    ...
                    clientId: undefined,
                    clientName: (parsedMetadata as any)?.clientName || undefined,
                    staffId: prismaTx.userId,
                    staffName: (parsedMetadata as any)?.staffName || undefined,
